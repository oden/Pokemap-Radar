...
- (void)setLocation:(Float64)latitude long:(Float64)longitude {
...
- (id)initWithFakePokemon:(Float64)latitude long:(Float64)longitude {
    self = [super init];
    if (self) {
        self.appearAnimation = kGMSMarkerAnimationPop;
        self.groundAnchor = CGPointMake(0.5, 0.5);
        self.position = CLLocationCoordinate2DMake(latitude, longitude);
        self.pokemonSpawnId = @"Fake";
        self.pokemonId = PokemonIdMew;
        self.timeExpireDate = ([[NSDate date] timeIntervalSince1970] * 1000) + ((60 * 60) * 1000);
